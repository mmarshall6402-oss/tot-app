...
// Rough IP estimate from wins+losses (each decision ≈ 5-6 IP per start, ~10-15 starts/decision)
// Falls back to null if no data.
function estimateIP(pitcher) {
  if (!pitcher) return null;
  const era  = parseFloat(pitcher.era);
  const whip = parseFloat(pitcher.whip);
  // If ERA and WHIP are both suspiciously extreme, assume tiny sample
  if (!isNaN(era) && !isNaN(whip)) {
    if (era === 0 || (era < 1.0 && whip > 1.3)) return 10;  // almost certainly < 20 IP
    if (era < 1.5) return 25;  // probably < 30 IP
  }
  return null;  // unknown — don't penalise
}
...
export function applyFilterLayer(pick, game, mlb, modelProb) {
  ...
  if (isAutoKill) {
    verdict = "PASS";
  } else if (trueEdgeFrac >= CLEAN_EDGE_THRESHOLD && variance !== "HIGH") {
    verdict = "CLEAN";
