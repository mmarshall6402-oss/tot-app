...
function _applyFilterLayer(pick, game, mlb, modelProb) {
  ...
  // Old formula shrunk toward 50/50 — fine for favorites (50% ≈ market) but broken
  // for underdogs (market 44% → even a fully-shrunk model at 50% is still 6pp above
  // market, producing spurious 12% edges). Shrinking toward the market anchor directly
  // fixes both cases: edges reflect only how much the model disagrees with the market,
  // not the model's absolute distance from 50%.
  //
  //   blend: LOW=0.45, MED=0.35, HIGH=0.25
  //   trueWinProb = market + (model − market) × blend
  //
  // Examples (LOW var):
  //   model 58%, market 44% (underdog) → 44 + 14×0.45 = 50.3%  → edge  6.3%
  //   model 65%, market 56% (favorite) → 56 +  9×0.45 = 60.1%  → edge  4.1%
  //   model 75%, market 60% (favorite) → 60 + 15×0.45 = 66.8%  → edge  6.8%
  const blend       = variance === "HIGH" ? 0.25 : variance === "MED" ? 0.35 : 0.45;
