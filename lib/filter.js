...
function _applyFilterLayer(pick, game, mlb, modelProb) {
  ...
  if (sharpImpliedP === null) {
    const baseWinProb = 0.5 + (pickModelProb - 0.5) * shrinkFactor;
    return {
      verdict: "PASS", confidence: 0, confidenceOf: 10,
      trueEdgePct: 0, rawEdgePct: 0,
      variancePenalty: 0, samplePenalty: 0, lineupPenalty: 0,
      trueWinProbPct: parseFloat((baseWinProb * 100).toFixed(1)),
      sharpImpliedPct: 50,
  ...
  // Underdog picks: shrink toward the market instead of toward 50%.
  // For favorites, 50% ≈ market so the standard formula is fine. For underdogs,
  // 50% is already 6–12pp above the market price, so shrinking toward 50% barely
  // moves the edge. The market is pricing in SP quality, lineup, and line action
  // the model underweights — so we blend toward market directly.
  //   underdogShrink = shrinkFactor × 0.55  →  LOW=0.43, MED=0.34, HIGH=0.25
  // Example: model 58%, market 44% (LOW var) → trueWinProb ≈ 50%, edge ≈ 6%
  //          vs old formula: trueWinProb 56.4%, edge 12.6%
  const isUnderdogPick = sharpImpliedP < 0.50 && pickModelProb > sharpImpliedP;
  const trueWinProb = isUnderdogPick
    ? sharpImpliedP + (pickModelProb - sharpImpliedP) * (shrinkFactor * 0.55)
    : 0.5 + (pickModelProb - 0.5) * shrinkFactor;
