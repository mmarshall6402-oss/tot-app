...
function andGate({
  ...
  // Minimum edge — cheap early gate; SNR check below is the binding constraint.
  //   ≥ 57% implied (~-133+) → 2.5% floor
  //   default               → 3.0% floor
  if (trueEdgePct < dynamicMinEdge)
    failures.push(`true edge ${trueEdgePct.toFixed(1)}% < ${dynamicMinEdge}%`);

  // Signal-to-noise — chalk markets are efficiently priced; heavy favorites need the
  // same SNR scrutiny as dogs. A 2% edge with 8% model uncertainty is noise regardless
  // of how "obvious" the direction is. Cap at 12% to avoid blocking legitimate large-edge picks.
...
function _applyFilterLayer(pick, game, mlb, modelProb) {
  ...
  // Dynamic edge floor: first-gate filter before the SNR check.
  // Chalk floors are set at 2.5% (above the 3% minimum model uncertainty floor)
  // so the SNR check remains the binding constraint — the edge floor is a cheap
  // early exit, not a substitute for signal quality scrutiny.
  const dynamicMinEdge = sharpImpliedP != null && sharpImpliedP >= 0.62 ? 2.5
                       : sharpImpliedP != null && sharpImpliedP >= 0.57 ? 2.5
                       : MIN_TRUE_EDGE * 100;
