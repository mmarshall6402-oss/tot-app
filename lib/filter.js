...
//
// Calibration notes:
//   LOW variance, avg data: ~7.0–8.0 (CLEAN range)
//   MED variance, avg data: ~6.0–6.8 (borderline BET)
//   HIGH variance: ~3.0–4.5 (always PASS — AND-gate blocks independently)
//
...
function computeConfidence({
  ...
  if (pickForm?.ops != null) {
    if (parseFloat(pickForm.ops) >= 0.760) { score += 0.3; reasons.push("+0.3 strong offensive form"); }
    if (parseFloat(pickForm.ops) <= 0.680) { score -= 0.3; reasons.push("-0.3 weak offensive form"); }
  }
  if (pickForm7d?.ops != null) {
    if (parseFloat(pickForm7d.ops) >= 0.790) { score += 0.2; reasons.push("+0.2 offense hot last 7 days"); }
  ...
  if (pickWP !== null) {
    if (pickWP >= 0.560)      { score += 0.4; reasons.push("+0.4 strong team record"); }
    else if (pickWP >= 0.530) { score += 0.2; reasons.push("+0.2 winning record"); }
  ...
  // Thin edge penalty only — large edges don't boost confidence (a big apparent edge
  // in a liquid market is as likely to be model noise as a real inefficiency).
...
export function applyFilterLayer(pick, game, mlb, modelProb) {
  ...
  // Shrink factors: filter receives modelProbRaw (not the 20%-calibrated value from the
  // picks route). LOW variance uses 1.0 — the raw model probability is the signal;
  // the 3% square-line compression already penalises square lines sufficiently.
  const shrinkFactor   = variance === "HIGH" ? 0.55 : variance === "MED" ? 0.80 : 1.0;
