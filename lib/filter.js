...
function computeConfidence({
  ...
  // Below 0.6: confidence multiplied by 0.6 — mixed signals suppress certainty.
  // Hard verdict overrides (< 0.5 → PASS, < 0.4 → TRAP) are applied in applyFilterLayer.
  if (agreement.normalized != null) {
    if (agreement.normalized < 0.6) {
      score *= 0.6;
      reasons.push(`×0.6 signal agreement low (${agreement.normalized.toFixed(2)}, ${agreement.sum}/${agreement.count})`);
    } else if (agreement.normalized >= 0.75) {
      score += 0.4;
      reasons.push(`+0.4 signals well aligned (${agreement.normalized.toFixed(2)}, ${agreement.sum}/${agreement.count})`);
    }
  }
...
export function applyFilterLayer(pick, game, mlb, modelProb) {
  ...
  // Hard verdict overrides from signal agreement.
  // normalized < 0.5: signals are net-negative → treat as PASS regardless of edge.
  // normalized < 0.4: majority of signals actively oppose the pick → TRAP warning.
  if (agreement.normalized != null) {
    if (agreement.normalized < 0.4) verdict = "TRAP";
    else if (agreement.normalized < 0.5 && verdict !== "TRAP") verdict = "PASS";
  }
