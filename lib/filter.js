...
const MIN_CONFIDENCE          = 7.0;   // precision target: higher bar = better win rate
...
function andGate({
  ...
  if (oppForm7dOps >= 0.800 && oppForm7dOps > oppFormOps)
  ...
  // Contra line movement — sharp money flows opposite the pick. Blocks CLEAN; BET still allowed
  // if everything else is solid. The market's collective information is the strongest trap signal.
  if (lineSignal === "contra")
    failures.push("contra line movement — sharp money opposes pick direction");

  // Signal agreement below CLEAN threshold. Split or conflicted signals mean independent
  // data sources disagree; we can't confidently pick a direction. BET still allowed.
  if (agreement.normalized != null && agreement.normalized < 0.55)
    failures.push(`signal agreement ${(agreement.normalized * 100).toFixed(0)}% < 55% threshold — mixed signals`);

  // No confirmed lineup from either team. We don't know who's batting, so lineup-vs-pitcher
  // handedness and Savant scores are approximations. Blocks CLEAN; BET still allowed.
  if (!mlb?.homeLineupSavant && !mlb?.awayLineupSavant)
    failures.push("no confirmed lineups — batting order unknown, lineup quality unverifiable");
...
function _applyFilterLayer(pick, game, mlb, modelProb) {
  ...
  //
  //   blend: LOW=0.38, MED=0.22, HIGH=0.12
  //
  ...
  //   model 65%, market 56% → 56 + 9×0.38 = 59.4%  → edge 3.4%
  //   model 70%, market 56% → 56 + 14×0.38 = 61.3%  → edge 5.3%
  // Examples (MED var, TBD SP):
  //   model 90%, market 50% → 50 + 40×0.11 = 54.4%  → rawEdge 4.4% → displayed ~2%
  const pickSPMissing = (pickIsHome ? homeP : awayP).flags.includes("NO_PITCHER_DATA");
  const blend         = (variance === "HIGH" ? 0.12 : variance === "MED" ? 0.22 : 0.38)
  ...
  if (failures.length === 0 && variance !== "HIGH") {
    ...
    const onlyMinorFailures = failures.every(f =>
      ...
      f.includes("contra line movement") ||  // sharp money contra: blocks CLEAN, BET survives
      f.includes("signal agreement") ||      // mixed signals: blocks CLEAN, BET survives
      f.includes("confirmed lineups")        // no lineup data: blocks CLEAN, BET survives
    );
    if (
      !catastrophic &&
      onlyMinorFailures &&
