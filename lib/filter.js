...
// ─── Constants ────────────────────────────────────────────────────────────────
//
// FILTER_PARAMS collects every continuous gate threshold (blend weights, edge
// floors, decay penalties) into one named, exported, live-read object —
// mirrors lib/probability.js's WEIGHTS pattern, so lib/backtest/filter-search.js
// can vary these programmatically the same way lib/backtest/weight-search.js
// varies WEIGHTS. Extraction is a pure rename: values are unchanged from the
// original inline literals. Binary AND-gate rules (Coors, juice, etc.) and
// pitcherFlags()/computeBullpenScore()/computeConfidence()'s internal scoring
// weights are deliberately NOT included — see the model-improvement plan.

export const FILTER_PARAMS = {
  SQUARE_LINE_COMPRESSION: 0.015, // 3% was eating ~40% of a real 7% edge
  MIN_CONFIDENCE: 6.5,            // recalibrated: +0.5 edge bonus removed, 7.0 was too tight
  MIN_IP_MEANINGFUL: 25,          // raised: need 25 IP before ERA is meaningful
  MIN_IP_BET: 12,                 // hard floor: < 12 IP = can't trust ERA at all → auto-PASS
  // Reduced from 3.0% → 1.5% to match compressed blend scale.
  // Old blend 0.38 could produce 10%+ raw edges; new blend 0.20 caps around 4-6%.
  // 1.5% is the meaningful signal floor in the new compressed range.
  MIN_TRUE_EDGE: 0.015,

  // Blend: model prob is pulled toward market by this fraction, keyed by variance.
  // Values below are the "Conservative candidate" from scripts/backtest/tune-filter.js:
  // commit 5aad788 cut these ~1.9x (0.38/0.22/0.12 -> 0.20/0.12/0.06) to correct a
  // measured -30.9pp overconfidence in the raw model. Isotonic calibration
  // (lib/probability.js's getCalibratedModelProbability, now wired into every live
  // pick route) closes ~60% of that overconfidence in filter.js's actual operating
  // band (50-66% implied probability, measured across 2022-2025 — thousands of
  // games). These values restore ~60% of the distance back toward the pre-5aad788
  // numbers to match — never fully reverting. See scripts/backtest/tune-filter.js
  // for the derivation and Tier 2/Tier 3 evidence.
  BLEND_LOW: 0.3081,
  BLEND_MED: 0.1848,
  BLEND_HIGH: 0.0924,
  BLEND_NO_PITCHER_MULT: 0.5, // halved when pick-side SP is TBD
  EDGE_CEILING_PP: 0.084,     // hard cap: trueWinProb never exceeds sharpImpliedP + this

  // Confidence-decay penalties applied to rawEdgePct.
  VARIANCE_PENALTY_HIGH: 1.5,
  VARIANCE_PENALTY_MED: 0.5,
  VARIANCE_PENALTY_LOW: 0,
  SAMPLE_PENALTY_NO_IP: 1.5,
  SAMPLE_PENALTY_TINY: 2.5,   // < MIN_IP_BET
  SAMPLE_PENALTY_SMALL: 1.0,  // < MIN_IP_MEANINGFUL
  LINEUP_PENALTY_MISSING: 0.4,

  // Dynamic minimum edge floor, tiered by how heavy a favorite the market sees.
  DYN_EDGE_TIER1_IMPLIED: 0.62,
  DYN_EDGE_TIER1_FLOOR: 0.8,
  DYN_EDGE_TIER2_IMPLIED: 0.57,
  DYN_EDGE_TIER2_FLOOR: 1.0,

  SNR_CAP_DEFAULT: 0.12,
};
...
function pitcherFlags(pitcher) {
  ...
  const era  = toNum(pitcher.era);
  const whip = toNum(pitcher.whip);
  const xfip = toNum(pitcher.xFip);
  const kbb  = toNum(pitcher.kBBPct);
  const hh   = toNum(pitcher.hardHitPct);
  const ip   = parseIP(pitcher.inningsPitched);

  const flags = [];

  if (ip !== null && ip < FILTER_PARAMS.MIN_IP_MEANINGFUL) flags.push("SMALL_SAMPLE");
...
function sharpImplied(pick, game) {
  ...
  const compression = isSquare ? FILTER_PARAMS.SQUARE_LINE_COMPRESSION : 0;
...
function computeConfidence({
  ...
  // ── Variance ──────────────────────────────────────────────────────────────
  // LOW bonus reduced 1.0→0.4: calibration showed LOW-var picks win 42% vs MED 50%.
  // HIGH confidence correlates with overconfidence; rewarding LOW var amplified it.
  let vAdj = 0;
  if (variance === "LOW")  { vAdj += 0.4;  reasons.push("+0.4 low variance"); }
  ...
  // Alignment bonus removed: perfectly-aligned signals mean the market has already
  // priced the pick in. Calibration showed high-agreement picks win less, not more.
  // Penalties are kept — signals opposing pick is still a genuine red flag.
  let agAdj = 0;
  if (agreement.normalized != null) {
    ...
    // No bonus for high agreement — when everything lines up, the market already knows.
  }
  ...
  if (pickIP !== null && pickIP >= FILTER_PARAMS.MIN_IP_MEANINGFUL) { pAdj += 0.4;  reasons.push("+0.4 meaningful SP sample"); }
  if (pickIP !== null && pickIP < FILTER_PARAMS.MIN_IP_BET)         { pAdj -= 2.0;  reasons.push("-2.0 SP tiny sample (<12 IP)"); }
  else if (pickIP !== null && pickIP < FILTER_PARAMS.MIN_IP_MEANINGFUL) { pAdj -= 1.2; reasons.push("-1.2 SP small sample"); }
  if (oppIP !== null && oppIP < FILTER_PARAMS.MIN_IP_BET)  { pAdj -= 0.6; reasons.push("-0.6 opp SP tiny sample"); }
  ...
  // ── Market ────────────────────────────────────────────────────────────────
  let mAdj = 0;
  // Confirming line bonus reduced 0.5→0.15: line confirmation was stacking on top of
  // already-aligned signals and pushing picks into the 7.5+ confidence bucket (0% win rate).
  if (lineSignal === "confirming") { mAdj += 0.15; reasons.push("+0.15 line confirms"); }
  if (lineSignal === "contra")     { mAdj -= 1.5; reasons.push("-1.5 line contradicts"); }
  if (trueEdgePct > 4.5 && lineSignal === "contra") {
    mAdj -= 0.8; reasons.push("-0.8 model disagreement: high claimed edge + contra line movement");
  }
  if (trueEdgePct <= 1.0) { mAdj -= 0.5; reasons.push("-0.5 thin true edge"); }
  if (trueEdgePct > 4.5) { mAdj -= 0.5; reasons.push("-0.5 edge skepticism (>4.5% unlikely in compressed model)"); }
...
function andGate({
  ...
  if (confidence < FILTER_PARAMS.MIN_CONFIDENCE)
    failures.push(`confidence ${confidence.toFixed(1)}/10 < ${FILTER_PARAMS.MIN_CONFIDENCE}`);
  ...
  if (pickIPGate !== null && pickIPGate < FILTER_PARAMS.MIN_IP_BET)
    failures.push(`pick-side SP < ${FILTER_PARAMS.MIN_IP_BET} IP (${pickIPGate.toFixed(0)} IP) — ERA unreliable`);
  ...
  // Model disagreement: >4.5% claimed edge while sharp money flows opposite = PASS
  if (trueEdgePct > 4.5 && lineSignal === "contra")
  ...
  const snrCap = dynamicMinEdge < FILTER_PARAMS.MIN_TRUE_EDGE * 100 ? dynamicMinEdge / 100 : FILTER_PARAMS.SNR_CAP_DEFAULT;
...
const SAFE_PASS = {
  ...
  trueWinProbPct: 50, sharpImpliedPct: 50, dynamicMinEdge: 1.5,
...
function _applyFilterLayer(pick, game, mlb, modelProb) {
  ...
  //
  //   blend: LOW=0.20, MED=0.12, HIGH=0.06
  //
  // Blends were reduced from 0.38/0.22/0.12 after calibration showed -30.9pp
  // overconfidence: when the model strongly disagreed with a sharp market, it
  // was usually wrong. Smaller blends keep trueWinProb inside the calibrated
  // 55–62% range where the model shows genuine signal.
  //
  ...
  //   model 65%, market 56% → 56 + 9×0.20 = 57.8%  → edge 1.8%
  //   model 75%, market 56% → 56 + 19×0.20 = 59.8%  → edge 3.8%
  // Examples (MED var, TBD SP):
  //   model 90%, market 50% → 50 + 40×0.06 = 52.4%  → rawEdge 2.4% → displayed ~1%
  const pickSPMissing = (pickIsHome ? homeP : awayP).flags.includes("NO_PITCHER_DATA");
  const blend         = (variance === "HIGH" ? FILTER_PARAMS.BLEND_HIGH : variance === "MED" ? FILTER_PARAMS.BLEND_MED : FILTER_PARAMS.BLEND_LOW)
                        * (pickSPMissing ? FILTER_PARAMS.BLEND_NO_PITCHER_MULT : 1.0);
  // Hard ceiling: never claim more than 6pp above the sharp implied probability.
  // This eliminates the badly-calibrated 65%+ bucket (28.6% actual vs 67% predicted).
  const rawBlend       = sharpImpliedP + (pickModelProb - sharpImpliedP) * blend;
  const trueWinProb    = Math.min(sharpImpliedP + FILTER_PARAMS.EDGE_CEILING_PP, rawBlend);
  ...
  const variancePenalty = variance === "HIGH" ? FILTER_PARAMS.VARIANCE_PENALTY_HIGH
                        : variance === "MED" ? FILTER_PARAMS.VARIANCE_PENALTY_MED
                        : FILTER_PARAMS.VARIANCE_PENALTY_LOW;
  const samplePenalty   = pickIP === null ? FILTER_PARAMS.SAMPLE_PENALTY_NO_IP
                        : pickIP < FILTER_PARAMS.MIN_IP_BET ? FILTER_PARAMS.SAMPLE_PENALTY_TINY
                        : pickIP < FILTER_PARAMS.MIN_IP_MEANINGFUL ? FILTER_PARAMS.SAMPLE_PENALTY_SMALL
                        : 0;
  const lineupOps = pickIsHome ? mlb?.homeLineupOpsVsPitcher : mlb?.awayLineupOpsVsPitcher;
  const lineupPenalty = lineupOps == null ? FILTER_PARAMS.LINEUP_PENALTY_MISSING : 0;
  ...
  // Floors reduced to match compressed blend scale (0.20 vs old 0.38).
  const dynamicMinEdge = sharpImpliedP != null && sharpImpliedP >= FILTER_PARAMS.DYN_EDGE_TIER1_IMPLIED ? FILTER_PARAMS.DYN_EDGE_TIER1_FLOOR
                       : sharpImpliedP != null && sharpImpliedP >= FILTER_PARAMS.DYN_EDGE_TIER2_IMPLIED ? FILTER_PARAMS.DYN_EDGE_TIER2_FLOOR
                       : FILTER_PARAMS.MIN_TRUE_EDGE * 100;
  ...
  // Confidence ceiling: ≥7.5 picks are capped at BET. Calibration showed that
  // 7.5–8.0 confidence = 0% win rate — "too perfect" setups are overconfident.
  let verdict;
  if (failures.length === 0 && variance !== "HIGH" && confidence < 7.5) {
    verdict = "CLEAN";
    ...
    // "model disagreement" is deliberately excluded here even though its text
    // also contains "edge": per the comment where that failure is created
    // (">7% claimed edge while sharp money flows opposite = PASS"), it must
    // block BET too, not just CLEAN.
    const onlyMinorFailures = failures.every(f =>
      !f.includes("model disagreement") && (
    ...
    if (
      ...
      confidence >= FILTER_PARAMS.MIN_CONFIDENCE
    ) {
      verdict = "BET";
    } else {
      verdict = "PASS";
