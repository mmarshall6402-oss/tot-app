...
const MIN_CONFIDENCE          = 6.5;   // out of 10
const MIN_IP_MEANINGFUL       = 20;    // innings pitched before trusting ERA/xFIP
const MIN_TRUE_EDGE           = 0.025; // 2.5% minimum after market calibration
...
function computeConfidence({
  ...
  // Recent offensive form (run production, not just pitching matchup)
...
function andGate({
  ...
  // Underdog filter — need higher confidence for dogs
  const pickOdds = pickIsHome ? game.homeOdds : game.awayOdds;
  if (pickOdds !== null && pickOdds > 0 && confidence < 8.0)
    failures.push(`underdog pick requires confidence ≥8 (have ${confidence.toFixed(1)})`);
...
export function applyFilterLayer(pick, game, mlb, modelProb) {
  ...
  if (failures.length === 0) {
    ...
    // BET: variance LOW/MED + positive edge + no catastrophic flags
    const catastrophic = failures.some(f =>
      f.includes("Coors") ||
      f.includes("juice") ||
      f.includes("closing line")
    );
    if (variance !== "HIGH" && trueEdgePct >= MIN_TRUE_EDGE * 100 && !catastrophic) {
      verdict = "BET";
    } else {
      verdict = "PASS";
    }
  ...
  // Half-size flag: CLEAN + meaningful edge but pick-side bullpen ERA > 6.00.
  // Edge is real; bullpen variance is also real — size down.
  const pickBullpen = pickIsHome ? mlb?.homeBullpen : mlb?.awayBullpen;
  const halfSize = verdict === "CLEAN" && trueEdgePct >= 2.5
    && pickBullpen?.era != null && parseFloat(pickBullpen.era) > 6.00;
