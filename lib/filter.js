...
// Additive. Starts at 4.5. Adjustments are grouped into five capped categories
...
const CONF_CAPS = {
  ...
  pitching: { min: -2.0, max:  1.5 },
  lineup:   { min: -1.0, max:  1.0 },
  bullpen:  { min: -1.5, max:  1.0 },
...
function computeConfidence({
  ...
  if (pickIP !== null && pickIP >= MIN_IP_MEANINGFUL) { pAdj += 0.5;  reasons.push("+0.5 meaningful IP"); }
  if (pickIP !== null && pickIP < MIN_IP_BET)         { pAdj -= 2.5;  reasons.push("-2.5 SP tiny sample (<12 IP)"); }
  else if (pickIP !== null && pickIP < MIN_IP_MEANINGFUL) { pAdj -= 1.5; reasons.push("-1.5 small sample"); }

  const oppIP = parseIP(mlb?.[pickIsHome ? "awayPitcher" : "homePitcher"]?.inningsPitched);
  if (oppIP !== null && oppIP < MIN_IP_BET) { pAdj -= 0.8; reasons.push("-0.8 opp SP tiny sample (<12 IP)"); }
  else if (oppIP !== null && oppIP < 20)    { pAdj -= 0.5; reasons.push("-0.5 opp SP small sample"); }

  if (pickedP.score > 0.45)       { pAdj += 0.6; reasons.push("+0.6 SP clear quality edge"); }
  else if (pickedP.score > 0.25)  { pAdj += 0.3; reasons.push("+0.3 SP quality edge"); }
  if (pickedP.score < -0.25)      { pAdj -= 0.8; reasons.push("-0.8 SP quality deficit"); }
  else if (pickedP.score < -0.1)  { pAdj -= 0.4; reasons.push("-0.4 SP below average"); }

  if (pickedP.flags.includes("ERA_XFIP_GAP"))     { pAdj -= 1.0; reasons.push("-1 ERA beats xFIP (regression risk)"); }
  if (pickedP.flags.includes("ERA_WHIP_MISMATCH")){ pAdj -= 0.5; reasons.push("-0.5 ERA/WHIP mismatch"); }
  if (pickedP.flags.includes("HIGH_ERA"))         { pAdj -= 1.0; reasons.push("-1 high ERA"); }
  ...
  if (bullEra != null) {
    if (bull.isRolling && bullEra < 3.50) { bAdj += 0.6;  reasons.push("+0.6 bullpen rolling edge"); }
    else if (bullEra < 3.80)              { bAdj += 0.35; reasons.push("+0.35 solid bullpen"); }
    if (bullEra > 5.00)                   { bAdj -= 0.7;  reasons.push("-0.7 bullpen struggling"); }
    else if (bullEra > 4.50)              { bAdj -= 0.4;  reasons.push("-0.4 bullpen concern"); }
  }
  if (bull?.whip != null) {
    const bullWhip = toNum(bull.whip);
    if (bullWhip != null && bullWhip < 1.15) { bAdj += 0.2; reasons.push("+0.2 bullpen elite WHIP"); }
    if (bullWhip != null && bullWhip > 1.55) { bAdj -= 0.3; reasons.push("-0.3 bullpen high WHIP"); }
  }
  if (bull?.fatigued) {
    bAdj -= 0.5;
  ...
  if (oppBull?.era != null) {
    ...
    if (oppBullEra != null && oppBullEra < 3.50 && oppBull.isRolling) {
      bAdj -= 0.4; reasons.push("-0.4 opponent has elite rolling bullpen");
    }
  }
  // SP+bullpen interaction — lives in bullpen bucket to avoid double-counting pitching penalties
  if (pickedP.score > 0.25 && bullEra != null && bullEra < 3.80) {
    bAdj += 0.4; reasons.push("+0.4 SP+bullpen alignment");
  }
  if (pickedP.score < -0.15 && bullEra != null && bullEra > 4.50) {
    bAdj -= 0.4; reasons.push("-0.4 SP+bullpen both weak");
  }

  const bCapped = clampAdj(bAdj, "bullpen");
  if (bCapped !== bAdj) reasons.push(`[bullpen cap: ${bAdj.toFixed(1)} → ${bCapped.toFixed(1)}]`);
  ...
  // ── Sum ───────────────────────────────────────────────────────────────────
  const score = 4.5 + vCapped + agAdj + pCapped + lCapped + bCapped + mCapped + fAdj;
...
export function applyFilterLayer(pick, game, mlb, modelProb) {
  ...
  const failures = andGate({
    ...
    trueWinProb, sharpImpliedP, agreement,
  });
