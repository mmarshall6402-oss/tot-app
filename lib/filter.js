...
function _applyFilterLayer(pick, game, mlb, modelProb) {
  ...
  //
  //   blend: LOW=0.25, MED=0.16, HIGH=0.10
  //
  // Rationale for 0.25: the MLB market is highly efficient. A 0.38 blend was saying
  // "our model captures 38% of what the market knows beyond the baseline," which
  // produced systematically inflated win probabilities. 0.25 is still meaningful
  // signal — it implies our model finds ~25% of realizable edge vs. market — but
  // is calibrated against actual observed win rates.
  //
  ...
  //   model 65%, market 56% → 56 + 9×0.25 = 58.3%  → edge 2.3%
  //   model 70%, market 56% → 56 + 14×0.25 = 59.5%  → edge 3.5%
  // Examples (MED var, TBD SP):
  //   model 90%, market 50% → 50 + 40×0.08 = 53.2%  → rawEdge 3.2% → displayed ~1%
  const pickSPMissing = (pickIsHome ? homeP : awayP).flags.includes("NO_PITCHER_DATA");
  const blend         = (variance === "HIGH" ? 0.10 : variance === "MED" ? 0.16 : 0.25)
  ...
  return {
    ...
    // Use decayed probability so calibration buckets see the same shrinkage as edge.
    // sharpImplied + displayEdge is the probability AFTER uncertainty penalties,
    // matching how we'd actually bet: we expect to win at the decayed-edge rate.
    trueWinProbPct:   parseFloat(Math.max(50, sharpImpliedP * 100 + displayEdgePct).toFixed(1)),
