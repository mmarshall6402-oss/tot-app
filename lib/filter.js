...
function computeConfidence({
  ...
  // Signal agreement — normalized 0→1 (0.5 = split, 1.0 = all agree).
  // Graduated penalty: mixed signals reduce confidence; strong alignment boosts it.
  // Hard TRAP override (< 0.4) applied in applyFilterLayer — majority opposing = trap.
  if (agreement.normalized != null) {
    if (agreement.normalized < 0.4)      { score -= 2.5; reasons.push(`-2.5 signals mostly oppose pick (${agreement.normalized.toFixed(2)})`); }
    else if (agreement.normalized < 0.5) { score -= 1.5; reasons.push(`-1.5 signals lean against pick (${agreement.normalized.toFixed(2)})`); }
    else if (agreement.normalized < 0.6) { score -= 0.8; reasons.push(`-0.8 signals split (${agreement.normalized.toFixed(2)}, ${agreement.sum}/${agreement.count})`); }
    else if (agreement.normalized >= 0.75) { score += 0.4; reasons.push(`+0.4 signals well aligned (${agreement.normalized.toFixed(2)}, ${agreement.sum}/${agreement.count})`); }
  }
