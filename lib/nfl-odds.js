...
export async function fetchNFLOdds(sportKey = "americanfootball_nfl") {
  ...
  if (TOA_KEY) {
    try {
      ...
      // TOA's account-wide quota (shared with MLB) — log off the h2h response,
      // no dedicated probe needed.
      logQuotaSnapshot("toa", h2hRes.headers.get("x-requests-remaining"), h2hRes.headers.get("x-requests-used"));
