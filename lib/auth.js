...
// Tuesday (America/Chicago) of the current fantasy week, as YYYY-MM-DD —
// fantasy weeks turn over Tuesday morning (after Monday Night Football),
// not the calendar-week Monday, so the free allowance should reset then too.
function weekStartDate(now = new Date()) {
  ...
  const dow = { Tue: 0, Wed: 1, Thu: 2, Fri: 3, Sat: 4, Sun: 5, Mon: 6 }[get("weekday")];
...
// smaller than the downside of a broken feature. The check-and-increment
// itself runs as a single atomic Postgres statement (see
// increment_fantasy_usage in sql/017_fantasy_free_usage.sql) rather than a
// read-then-write from here, so two concurrent requests can't both read
// "2 used" and both get waved through as the 3rd call.
export async function requireFantasyCallAllowance(request) {
  ...
  try {
    ...
    const { data, error: rpcError } = await supabase.rpc("increment_fantasy_usage", {
      p_user_id: user.id, p_week_start: weekStart, p_limit: FREE_STARTSIT_WEEKLY_LIMIT,
    });
    ...
    if (allowed === false) {
      return {
        user: null,
        error: Response.json({
          error: `Free plan includes ${FREE_STARTSIT_WEEKLY_LIMIT} Start/Sit calls per week — you've used them all. Resets Tuesday, or go Pro for unlimited calls plus the prop-line comparison.`,
        }, { status: 403 }),
      };
    }
  } catch (e) {
