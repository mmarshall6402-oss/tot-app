...
function buildNFLGamePicks(game, nfl) {
  ...
  if (game.spread != null && game.homeSpreadOdds != null && game.awaySpreadOdds != null) {
    ...
    const pickCoverProb = pick === game.homeTeam ? homeCoverProb : 1 - homeCoverProb;
    picks.push({
      ...base,
      ...
      modelProb: Math.round(pickCoverProb * 100),
      pick, edge: filter.trueEdgePct, isBet: filter.verdict === "BET",
      tier: deriveTier(filter), filter,
    });
  }
...
export function buildNFLModelPickRows(picks, date, seasonType = "regular") {
  return picks.map(p => ({
    date,
    game_id: p.gameId,
...
// ─── Grading (shared by app/api/cron/nfl-resolve and app/api/tracker/resolve) ────

export function findNFLGameMatch(pick, games) {
...
export function gradeNFLPick(pick, homeScore, awayScore) {
