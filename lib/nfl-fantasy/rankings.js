...
export function buildRankings(playersById, { targetSeason, format, leagueConfig = DEFAULT_LEAGUE_CONFIG, vorpOptions, ageById } = {}) {
  ...
  for (const [playerId, games] of playersById) {
    ...
    const age = ageById?.get(playerId);
    const projection = buildPlayerProjection(games, { asOfSeason: targetSeason, position, format, age });
