...
// Rank-slot penalty for a live injury designation — without this, an
// "Out"/"Doubtful" player's raw rank_overall could still put them at the
// top of Best Available (and Top Recommendation's "Draft to My Team" CTA)
// even while the very same row carries a FADE verdict and an Out badge
// right next to it. Same status vocabulary as verdict.js's injuryPenalty
// and NFLSection.js's INJURY_SEVERITY (ESPN/Sleeper free text varies, so
// this matches the designation itself rather than a source-specific enum).
// Out gets pushed roughly a full redraft round down — enough that healthy
// alternatives at the same rank win the recommendation — while
// Questionable's small penalty is closer to a tiebreaker than a real
// demotion, since most players who suit up end up playing meaningful snaps.
const INJURY_RANK_PENALTY = { out: 40, doubtful: 15, questionable: 3 };
function injuryRankPenalty(status) {
  if (!status) return 0;
  const s = status.toLowerCase();
  if (/\b(out|ir|pup|nfi|susp)\b/.test(s)) return INJURY_RANK_PENALTY.out;
  if (/doubtful/.test(s)) return INJURY_RANK_PENALTY.doubtful;
  if (/questionable/.test(s)) return INJURY_RANK_PENALTY.questionable;
  return 0;
}
  ...
  return available
    .map((p) => {
      ...
      return {
        ...p,
        needSlots,
        recommendScore: (p.rank_overall ?? 999) - needSlots * NEED_BONUS_PER_SLOT + injuryRankPenalty(p.injury_status),
