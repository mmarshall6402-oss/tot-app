...
// week: from lib/nfl-fantasy/season.js's fetchCurrentNflWeek() — null when
// SportsData.io is unavailable, same graceful-miss posture as everywhere
// else this app calls that provider. A null week can never be resolved by
// app/api/cron/nfl-fantasy-resolve (which matches on exact week), so it
// just stays pending rather than risking a wrong grade against the wrong
// week's box scores.
export async function logLiveProbabilityCall({ rowA, rowB, probability, scoringFormat, season, week }) {
  try {
