...
export function runFantasyBacktest({ playerStatsRows, playersRows, targetSeason, format = "ppr", leagueConfig = DEFAULT_LEAGUE_CONFIG, topN = 12 }) {
  ...
  const modelRanking = buildRankings(playersById, { targetSeason, format, leagueConfig, ageById });
  const plainVorpRanking = buildRankings(playersById, { targetSeason, format, leagueConfig, vorpOptions: { ceilingWeight: 0 }, ageById });
