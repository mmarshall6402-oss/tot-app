...
export function buildPlayerProjection(weeklyGameLogs, { asOfSeason, position, age, format = "ppr" }) {
  ...
  let weightedSum = 0;
  let weightTotal = 0;
  seasonPpg.slice(0, SEASON_WEIGHTS.length).forEach((s, i) => {
    const w = SEASON_WEIGHTS[i];
  ...
  const avgGamesPlayed = relevantSeasons.reduce((sum, s) => sum + s.gamesPlayed, 0) / relevantSeasons.length;
