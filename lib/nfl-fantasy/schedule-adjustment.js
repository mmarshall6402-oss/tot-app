...
//
// The uploaded grids seen so far measure opponent run-defense strength
// specifically, not overall matchup difficulty — a real signal for a
// rushing workload, but not for a receiving one. So the adjustment is
// scaled down by position: full weight for RB, partial for QB (rushing
// production), none for WR/TE (a tough run defense says nothing about
// pass-defense strength).
import { normalizeName } from "./id-map.js";
import { assignTiers } from "./tiers.js";

const PLAYOFF_WEEKS = [15, 16, 17];
// Exported for lib/nfl-fantasy/teammate-impact.js and the in-season tracker
// (app/api/nfl/fantasy/tracker), which read every week from
// nfl_fantasy_schedule_difficulty rather than just the playoff-weeks slice
// this file uses for the draft-ranking nudge.
export const DIFFICULTY_SCORE = { easy: 1, neutral: 0, hard: -1, bye: -1 };
const SCHEDULE_ADJUSTMENT_SCALE = 5;
const POSITION_RELEVANCE = { RB: 1, QB: 0.3, WR: 0, TE: 0 };
...
export function applyScheduleAdjustment(ranked, adjustmentByName) {
  const adjusted = ranked.map((p) => {
    ...
    const scheduleAdjustment = scheduleAdjustmentFor(p.name, adjustmentByName) * relevance;
