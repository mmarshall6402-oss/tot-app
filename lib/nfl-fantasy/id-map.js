...
// Shared join key across every name-based crosswalk in lib/nfl-fantasy/*
// (nflverse <-> ESPN <-> Sleeper all spell names slightly differently).
// Diacritics are stripped first (NFD-decompose, then drop the combining
// marks) so e.g. nflverse's "Ollie Gordon" vs a source spelling a name with
// an accent still collide — a real, observed mismatch class, not a
// hypothetical: two sources agreeing on every letter except one accent
// otherwise fails silently right up through the live draft board (a picked
// player just never leaves "available," with nothing to explain why).
// Internal whitespace is collapsed too, since a stray double space from a
// scraped source is invisible in normal debugging but breaks equality just
// as completely as an accent does.
