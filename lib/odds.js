...
async function fetchFromTheOddsAPI() {
  ...
  // Captured regardless of res.ok — a 401/429 still carries these headers and is
  // often the first sign of quota exhaustion, which is exactly what the feed
  // adapter's credit logging (lib/feed-adapter/credit-log.js) needs to see.
  _lastTOACredits = {
...
// Most recent TOA response's x-requests-used/x-requests-remaining, for the feed
// adapter's mandatory predicted-vs-actual credit logging (see lib/feed-adapter/
// mlb-moneyline.js). Read-only observability — does not affect fetchMLBOdds()'s
// behavior or return shape.
//
// This is a per-instance module var, not a source of truth: it only reflects the
// last call made by THIS warm lambda. odds_credit_log (Supabase) is the actual
// account-wide ledger; nothing should read this as "current remaining credits."
let _lastTOACredits = null;
export function getTOACreditHeaders() { return _lastTOACredits; }
