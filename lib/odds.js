...
function mergeOdds(base, supplement) {
  ...
  if (added.length) console.log(`[odds] SGO added ${added.length} extra games`);
  return [...base, ...added];
}
...
async function _fetchFresh() {
  ...
  // Supplement with SGO to pick up any games TOA missed
  try {
    const sgo = await fetchFromSGO();
    if (sgo.length) results = mergeOdds(results, sgo);
  } catch (e) {
    console.warn("[odds] SGO supplement failed:", e.message);
  }
