...
import { logQuotaSnapshot } from "./quota-log.js";
...
async function fetchFromTheOddsAPI() {
  ...
  logQuotaSnapshot("toa", res.headers.get("x-requests-remaining"), res.headers.get("x-requests-used"));
...
async function _fetchFresh() {
  ...
  // Kalshi overlay runs last and wins on any game it has a market for —
  // thinner spread than a sportsbook's vig, so it's the sharper implied-
  // probability signal when available. Falls through to the sportsbook
  // chain above untouched for every other game (no Kalshi market, or the
  // fetch/match failed) — see lib/kalshi-odds.js for the fail-soft posture.
  if (results.length) {
    results = await overlayKalshiImplied(results, "mlb");
  }
