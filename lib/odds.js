...
async function _fetchFresh() {
  ...
  // Primary succeeded — parse, then supplement with ESPN for any uncovered games
  if (res.ok) {
    const json = await res.json();
    if (!json?.error || json?.data?.length) {
      ...
      // Supplement primary with ESPN to catch any games primary missed
      try {
        const espn = await fetchFromESPN(today);
        results = mergeOdds(results, espn);
      } catch (e) {
        console.warn("[odds] ESPN supplement failed:", e.message);
      }
      _cache = results;
      _cacheTime = Date.now();
      console.log(`[odds] primary + ESPN supplement — ${results.length} games cached`);
      return results;
    }
  ...
  // Fallback chain: try The Odds API, supplement gaps with SportsData.io
  let fallback = [];
  try {
    fallback = await fetchFromTheOddsAPI(today);
  } catch (e) {
    console.warn("[odds] The Odds API failed:", e.message);
  }

  // Always supplement from SportsData.io to fill any coverage gaps
  try {
    const sd = await fetchFromSportsData(today);
    fallback = mergeOdds(fallback, sd);
  } catch (e) {
    console.warn("[odds] SportsData.io supplement failed:", e.message);
  }

  // ESPN — free, no key, covers every scheduled game (last sweep before giving up)
  try {
    const espn = await fetchFromESPN(today);
    fallback = mergeOdds(fallback, espn);
  } catch (e) {
    console.warn("[odds] ESPN supplement failed:", e.message);
  }
