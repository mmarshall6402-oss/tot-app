...
// Most recent per-event response's x-requests-used/x-requests-remaining, for the
// feed adapter's mandatory credit logging (see lib/feed-adapter/mlb-props.js).
// Read-only observability — does not affect fetchEventPlayerProps()'s behavior.
//
// Per-instance module var, not a source of truth — see the identical caveat on
// lib/odds.js's getTOACreditHeaders(). odds_credit_log is the real ledger.
let _lastCredits = null;
export function getPropsCreditHeaders() { return _lastCredits; }
