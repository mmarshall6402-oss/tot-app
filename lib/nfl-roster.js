...
// Hyphens/apostrophes/periods vary by source and by how someone types a name
// they've only heard spoken (e.g. "Jaxon Smith-Njigba" typed as "Smith
// Njigba") — treat them as spaces so punctuation never blocks an otherwise
// exact match. Applied to both sides of every comparison below.
...
function bestMatch(index, query) {
  ...
  for (const [name, player] of index) {
    ...
    if (n.includes(q) || q.includes(n)) return player;
  }
  const lastWord = q.split(" ").pop();
  for (const [name, player] of index) {
    if (normalizeForMatch(name).split(" ").pop() === lastWord) return player;
  }
  return null;
}
...
// someone actually typed for. Callers pass normalizeForMatch'd strings on
// both sides.
...
// PostgREST caps a single select at its db-max-rows setting (1000 by
// default) regardless of how many rows actually match — a full 32-team NFL
// roster crawl (53-man rosters, ~1700 players) sails past that silently, no
// error, just a truncated `data` array. Page through with .range() so every
// row actually gets scored instead of only whichever ~1000 sorted first.
async function fetchAllRows(supabase, table, sport) {
  const PAGE = 1000;
  const rows = [];
...
// Reads the Supabase-cached index (refreshed daily by
...
// external call at request time left to fail.
// ESPN's headshot CDN follows a predictable id-keyed path — the
// player_index table stores this same ESPN athlete id as player_id (see
// buildPlayerIndex above), so search results can link straight to a photo
// with no extra fetch. next.config.mjs already whitelists this host/path
// for next/image. Missing/broken photos fall back to PlayerHeadshot's
// initials avatar rather than needing a null-check at every call site.
export function nflHeadshotUrl(id) {
...
export async function searchNFLPlayers(query, limit = 8) {
  ...
  try {
    ...
    const data = await fetchAllRows(supabase, "player_index", "nfl");
    const scored = [];
    for (const row of data) {
      const score = matchScore(normalizeForMatch(row.name_lower), q);
      if (score !== null) {
