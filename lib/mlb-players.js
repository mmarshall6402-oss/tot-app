...
// default) regardless of how many rows actually match. 30 teams at the
// 26-man active limit stays under that, but September call-ups (40-man
// rosters active) push it to ~1200 — page through with .range() so a
// September search doesn't silently start dropping players again.
async function fetchAllRows(supabase, table, sport, columns) {
  const PAGE = 1000;
  const rows = [];
...
export async function searchMLBPlayers(query, limit = 8) {
  ...
  try {
    ...
    const data = await fetchAllRows(supabase, "player_index", "mlb", "player_id, name, name_lower, team, position");
    const scored = [];
...
async function fetchBio(id) {
  ...
  return {
    ...
    // MLB's static headshot CDN — serves a generic silhouette for players
    // without a photo on file, so this is never a broken image, just
    // sometimes a placeholder.
    headshot: `https://midfield.mlbstatic.com/v1/people/${p.id}/spots/120`,
  };
}
