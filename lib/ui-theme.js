...
// Filled pill tab/category switcher — rounded-full, solid accent when active,
// outlined dark chip when inactive. One recipe shared by every sport/category
// filter row in the app (games/portfolio sub-nav, schedule, teams, players).
export function tabButtonStyle({ active = false, accent = t.color.brand } = {}) {
  return {
    padding: "7px 13px",
    fontSize: 13,
    fontWeight: 600,
    color: active ? "#0b0c10" : t.color.textSecondary,
    background: active ? accent : t.color.surfaceRaised,
...
// Section header with a leading glyph — the "★ Favorites" pattern grouping
// related rows (top picks, schedule day groups, etc).
export function sectionLabelStyle() {
  return {
    display: "flex", alignItems: "center", gap: 7,
    fontSize: 13, fontWeight: 700, color: t.color.textPrimary,
...
// Small square icon button (sort/refresh/etc) — active state is a light tint
// + colored icon, not a solid fill, so it stays consistent with the
// underline-tab language instead of reading as a bold on/off switch.
export function iconButtonStyle({ active = false, accent = t.color.brand } = {}) {
  return {
    width: 32,
    height: 32,
    borderRadius: t.radius.sm,
    ...
    background: active ? `${accent}1a` : t.color.surface,
    border: `1px solid ${active ? accent : t.color.border}`,
...
export const S = {
  ...
  nav: { display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 20px", borderBottom: `1px solid ${t.color.border}`, position: "sticky", top: 0, background: "rgba(11,12,16,0.97)", backdropFilter: "blur(12px)", zIndex: 100, boxShadow: t.shadow.chrome },
  ...
  carousel: { margin: "8px 20px 6px", border: `1px solid ${t.color.border}`, borderRadius: t.radius.lg + 4, padding: "14px 16px", background: t.color.surface, height: 94, overflow: "hidden" },
  carouselTag: { fontSize: 9, fontWeight: 700, color: t.color.brand, letterSpacing: 2, marginBottom: 5 },
  carouselMatchup: { fontFamily: t.font.mono, fontSize: 15, fontWeight: 700 },
  dateScroll: { display: "flex", gap: 6, padding: "8px 20px", overflowX: "auto" },
  dateBtn: { flexShrink: 0, padding: "6px 12px", borderRadius: 999, fontSize: 12, fontWeight: 600, border: "1px solid", letterSpacing: 0.2, whiteSpace: "nowrap", transition: t.transition },
  subNav: { display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, padding: "8px 20px" },
  ...
  content: { flex: 1, padding: "8px 20px 84px", display: "flex", flexDirection: "column", gap: 10, overflowY: "auto" },
  ...
  badge: { display: "inline-block", fontSize: 11, fontWeight: 700, padding: "3px 10px", borderRadius: 999, letterSpacing: 0.3, marginBottom: 6 },
  ...
  saveBtn: { fontSize: 11, fontWeight: 600, padding: "7px 13px", borderRadius: t.radius.sm, border: "1px solid", letterSpacing: 0.2, transition: t.transition },
  ...
  pitchRow: { display: "flex", alignItems: "center", gap: 8, marginTop: 10, paddingTop: 10, borderTop: `1px solid ${t.color.border}` },
  pitchBox: { flex: 1 },
