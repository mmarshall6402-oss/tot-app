...
export async function getMLBTeams() {
  ...
  const res = await fetch(`${MLB_API}/teams?sportId=1&hydrate=division`, { signal: AbortSignal.timeout(5000) });
...
export async function getNFLTeams() {
  ...
  const res = await fetch(`${ESPN_NFL}/teams?limit=40`, { signal: AbortSignal.timeout(5000) });
