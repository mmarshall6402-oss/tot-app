...
async function fetchOneEvent(supabase, eventId) {
  return withLease(
    supabase,
    `props:${eventId}`,
    // ttlSeconds must exceed this fetch's worst-case latency, with margin — the lease
    // has no heartbeat/renewal (see lib/feed-adapter/lease-lock.js), so a fetch that
    // outlives its own lease's TTL causes a real double-spend, proven in the Phase 0
    // race test (testC). 30s is a wide margin over a single-event TOA call today;
    // check max(duration_ms) in odds_credit_log where adapter_key = 'mlb_props'
    // periodically rather than assuming this holds forever.
    { ttlSeconds: 30 },
    async () => {
