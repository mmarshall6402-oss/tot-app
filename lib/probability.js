...
// Empirical MLB home win rate 2022-2024: 52.1-53.3% across 7,289 games.
// 35 Elo points predicts 55.0% for equal-strength teams — overcounts by ~2%.
// 20 Elo points predicts 52.9%, which matches the observed data.
...
// 2026 MLB league ERA tracking ~4.0-4.1 (down from 4.20 used in prior seasons).
// xFIP historically runs ~0.35 below ERA; shifting proportionally.
const LEAGUE_ERA  = 4.10;
const LEAGUE_WHIP = 1.27;
const LEAGUE_XFIP = 3.75;
const LEAGUE_KBB  = 8.5;   // league avg K-BB% ~8–9%, stable
const LEAGUE_HH   = 35.5;  // league avg hard-hit% ~35%, stable
...
// ERA range [2.10, 6.10] → midpoint 4.10 = LEAGUE_ERA  ✓ (2026: ERA ~4.0-4.1)
// xFIP range [1.75, 5.75] → midpoint 3.75 = LEAGUE_XFIP ✓ (xFIP ≈ ERA - 0.35)
...
function pitcherScoreFromStats(era, xfip, whip, kbb, hh, ip) {
  ...
  // xFIP has its own range centred on LEAGUE_XFIP = 3.75; ERA uses [2.10, 6.10]
  const eraNorm  = xfip != null
    ? norm(sXfip, 1.75, 5.75) * -1   // xFIP range [1.75, 5.75] midpoint = 3.75
    : norm(sEra,  2.10, 6.10) * -1;  // ERA: lower is better
...
function pitcherScore(pitcher) {
  ...
  if (r && r.numStarts >= 2 && r.ip >= 6) {
    ...
    // 50% season (stability) + 50% recent (form). ERA over 2-5 starts is high-variance;
    // equal weighting prevents a single blow-up start from dominating the score.
    return seasonScore * 0.50 + recentScore * 0.50;
  }
