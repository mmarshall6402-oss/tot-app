...
function pitcherScoreFromStats(era, xfip, whip, kbb, hh, ip) {
  ...
  const eraNorm  = norm(qualityEra, 2.0, 6.0) * -1;
  const whipNorm = norm(sWhip, 0.9, 1.7) * -1;
  const kbbNorm  = norm(sKbb, 2.0, 16.0);
  const hhNorm   = norm(sHh,  26.0, 46.0) * -1;
...
function pitcherScore(pitcher) {
  ...
  const seasonScore = pitcherScoreFromStats(
    parseFloat(pitcher.era)        ?? null,
    parseFloat(pitcher.xFip)       ?? null,
    parseFloat(pitcher.whip)       ?? null,
    parseFloat(pitcher.kBBPct)     ?? null,
    parseFloat(pitcher.hardHitPct) ?? null,
    ip
  );
