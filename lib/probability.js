...
export function getModelProbability(game, mlb) {
  ...
  // 35% — Starting pitcher: xFIP is the primary signal, K-BB% secondary, ERA tertiary.
  // Raised significantly: SP is the single most predictive pre-game variable in MLB.
  // Diminishing returns cap: beyond a 0.8 score differential, additional separation adds
  // only 25% of its face value. Prevents "ace vs rookie" from dominating all other signals.
  const homePScore = pitcherScore(mlb.homePitcher);
  const awayPScore = pitcherScore(mlb.awayPitcher);
  const spDiffRaw  = homePScore - awayPScore;
  const capAt      = 0.80;
  const spDiffCapped = Math.sign(spDiffRaw) * (
    Math.min(Math.abs(spDiffRaw), capAt) +
    Math.max(0, Math.abs(spDiffRaw) - capAt) * 0.25
  );
  const pitcherAdj = spDiffCapped * 0.175;
