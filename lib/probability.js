...
import { isotonicPredict } from "./backtest/metrics.js";

// ─── Tunable weight constants ─────────────────────────────────────────────────
// Extracted so lib/backtest/weight-search.js can vary them programmatically
// without touching formulas. Values are unchanged from the original inline
// literals — this block is a pure refactor, not a retune.

export const WEIGHTS = {
  // Elo prior
  HOME_ADVANTAGE: 35,

  // pitcherScoreFromStats() sub-weights (with hard-hit% available)
  PITCHER_ERA_W: 0.35,
  PITCHER_KBB_W: 0.30,
  PITCHER_WHIP_W: 0.20,
  PITCHER_HH_W: 0.15,
  // pitcherScoreFromStats() sub-weights (no hard-hit% available)
  PITCHER_ERA_W_NOHH: 0.40,
  PITCHER_KBB_W_NOHH: 0.35,
  PITCHER_WHIP_W_NOHH: 0.25,

  // pitcherScore() season vs. recent-starts blend
  PITCHER_SEASON_BLEND: 0.40,
  PITCHER_RECENT_BLEND: 0.60,

  // bullpenScore() sub-weights
  BULLPEN_ERA_W: 0.45,
  BULLPEN_WHIP_W: 0.35,
  BULLPEN_K9_W: 0.20,
  BULLPEN_ROLLING_BONUS: 1.1,
  BULLPEN_FATIGUE_CAP: 0.20,
  BULLPEN_FATIGUE_SCALE: 0.08,

  // lineupSavantScore() / lineup blend
  LINEUP_HANDEDNESS_W: 0.75,
  LINEUP_SAVANT_W: 0.25,

  // formScore() 10-day vs 7-day blend
  FORM_10D_W: 0.70,
  FORM_7D_W: 0.30,

  // getModelProbability() top-level factor scales
  PITCHER_DIFF_SCALE: 0.175,
  PITCHER_TANH_SATURATION: 0.8,
  PITCHER_TANH_AMPLITUDE: 0.8,
  LINEUP_DIFF_SCALE: 0.125,
  BULLPEN_DIFF_SCALE: 0.125,
  FORM_DIFF_SCALE: 0.022,
  HR_FACTOR_SCALE: 0.012,
  ELO_DIFF_SCALE: 0.11,
...
const HOME_ADVANTAGE = WEIGHTS.HOME_ADVANTAGE;
...
// ─── Isotonic calibration curve (process-local cache, same pattern as Elo) ──

let _calibrationCurve = null;

// Called by API routes after fetching the active curve from Supabase via
// lib/calibration-db.js. curve = { controlPoints: [{x,y}, ...] } or null.
export function setCalibrationCurve(curve) {
  _calibrationCurve = curve;
}

function loadSeedCalibrationCurve() {
  try {
    return JSON.parse(readFileSync(join(process.cwd(), "data/calibration/mlb_isotonic_v1.json"), "utf8"));
  } catch {
    return null;
  }
}

// getModelProbability's raw output, passed through isotonic recalibration.
// Falls back to the static seed file if no live curve has been set for this
// process/request (mirrors loadElo()'s static-file fallback). If neither is
// available, returns the raw probability unchanged — calibration is a
// correction layer, never a hard dependency for producing a probability.
export function getCalibratedModelProbability(game, mlb) {
  const raw = getModelProbability(game, mlb);
  const curve = _calibrationCurve ?? loadSeedCalibrationCurve();
  if (!curve?.controlPoints?.length) return raw;
  return clamp(isotonicPredict(curve.controlPoints, raw), WEIGHTS.PROB_MIN, WEIGHTS.PROB_MAX);
}
...
function pitcherScoreFromStats(era, xfip, whip, kbb, hh, ip) {
  ...
  if (hh != null) {
    return eraNorm * WEIGHTS.PITCHER_ERA_W + kbbNorm * WEIGHTS.PITCHER_KBB_W
      + whipNorm * WEIGHTS.PITCHER_WHIP_W + hhNorm * WEIGHTS.PITCHER_HH_W;
  }
  return eraNorm * WEIGHTS.PITCHER_ERA_W_NOHH + kbbNorm * WEIGHTS.PITCHER_KBB_W_NOHH
    + whipNorm * WEIGHTS.PITCHER_WHIP_W_NOHH;
}
...
function pitcherScore(pitcher) {
  ...
  if (r && r.numStarts >= 2 && r.ip >= 6) {
    ...
    return seasonScore * WEIGHTS.PITCHER_SEASON_BLEND + recentScore * WEIGHTS.PITCHER_RECENT_BLEND;
  }
...
function bullpenScore(bullpen) {
  ...
  const weight = bullpen.isRolling ? WEIGHTS.BULLPEN_ROLLING_BONUS : 1.0;
  ...
  const base = (eraNorm * WEIGHTS.BULLPEN_ERA_W + whipNorm * WEIGHTS.BULLPEN_WHIP_W + k9Norm * WEIGHTS.BULLPEN_K9_W) * weight;
  ...
  if (bullpen.fatigued && bullpen.eraInflation > 0) {
    return base - Math.min(WEIGHTS.BULLPEN_FATIGUE_CAP, bullpen.eraInflation * WEIGHTS.BULLPEN_FATIGUE_SCALE);
  }
  return base;
}
...
// Extracts the raw per-factor differentials (home - away, pre-weight-scale)
// that getModelProbability() blends together. Split out so the exact same
// blend can be re-run against a candidate WEIGHTS vector without needing
// the original mlb stats object again — see computeProbabilityFromComponents
// below and lib/backtest/live-weight-rows.js, which stores these components
// on every live pick specifically so weight-tuning can replay them later
// against real outcomes instead of only historical backtest data.
export function getModelProbabilityComponents(game, mlb) {
  const eloPrior = eloProb(game.homeTeam, game.awayTeam);
  const parkAdj = parkWinAdj(game.homeTeam);

  if (!mlb) {
    return { hasMlb: false, eloPrior, parkAdj };
  }

  const homePitcherScore = pitcherScore(mlb.homePitcher);
  const awayPitcherScore = pitcherScore(mlb.awayPitcher);
  ...
  const homeLineupBlend = hasSavant ? homeLineup * WEIGHTS.LINEUP_HANDEDNESS_W + homeSavant * WEIGHTS.LINEUP_SAVANT_W : homeLineup;
  const awayLineupBlend = hasSavant ? awayLineup * WEIGHTS.LINEUP_HANDEDNESS_W + awaySavant * WEIGHTS.LINEUP_SAVANT_W : awayLineup;
  ...
  const homeFormBlend = homeForm10 * WEIGHTS.FORM_10D_W + homeForm7 * WEIGHTS.FORM_7D_W;
  const awayFormBlend = awayForm10 * WEIGHTS.FORM_10D_W + awayForm7 * WEIGHTS.FORM_7D_W;

  const avgHrFactor = getHrFactor(game.homeTeam);

  return {
    hasMlb: true,
    eloPrior, parkAdj, avgHrFactor,
    homePitcherScore, awayPitcherScore,
    homeLineupBlend, awayLineupBlend,
    homeBull, awayBull,
    homeFormBlend, awayFormBlend,
  };
}

// Pure recombination: components (from getModelProbabilityComponents) + a
// weights vector -> a home-win probability. Takes an explicit weights arg
// (defaulting to live WEIGHTS) so lib/backtest/score-weights.js can score
// the exact same formula under alternative candidate vectors.
export function computeProbabilityFromComponents(c, weights = WEIGHTS) {
  if (!c.hasMlb) {
    const eloAdj = (c.eloPrior - 0.5) * weights.ELO_DIFF_SCALE;
    return clamp(0.5 + eloAdj + c.parkAdj, weights.PROB_MIN, weights.PROB_MAX);
  }
  ...
  const pitcherAdj = (() => {
    const raw = (c.homePitcherScore - c.awayPitcherScore) * weights.PITCHER_DIFF_SCALE;
    return Math.sign(raw) * Math.tanh(Math.abs(raw) / weights.PITCHER_TANH_SATURATION) * weights.PITCHER_TANH_AMPLITUDE;
  })();

  // 25% — Lineup vs pitcher handedness + Savant quality blend.
  const lineupAdj = (c.homeLineupBlend - c.awayLineupBlend) * weights.LINEUP_DIFF_SCALE;

  // 25% — Bullpen: relievers pitch ~40% of outs; equal weight to starter is correct.
  const bullAdj = (c.homeBull - c.awayBull) * weights.BULLPEN_DIFF_SCALE;

  // 4% — Recent form: deliberately capped, noise indicator.
  const formAdj = (c.homeFormBlend - c.awayFormBlend) * weights.FORM_DIFF_SCALE;

  // 5% — Park factor (run environment)
  const hrAdj = (c.avgHrFactor - 1.0) * weights.HR_FACTOR_SCALE;

  // 11% — Elo: team baseline strength absorbs the standings signal.
  const eloAdj = (c.eloPrior - 0.5) * weights.ELO_DIFF_SCALE;

  const total = 0.5 + pitcherAdj + lineupAdj + bullAdj + formAdj + c.parkAdj + hrAdj + eloAdj;
  return clamp(total, weights.PROB_MIN, weights.PROB_MAX);
}
...
export function getModelProbability(game, mlb) {
  return computeProbabilityFromComponents(getModelProbabilityComponents(game, mlb), WEIGHTS);
}

// Called by API routes after fetching the active weight vector from Supabase
// via lib/weights-db.js. Mirrors setEloRatings/setCalibrationCurve — merges
// overrides into the live WEIGHTS object read at call time by every function
// above. Partial vectors are fine; unspecified keys keep their current value.
export function setWeights(overrides) {
  if (!overrides) return;
