...
export function getModelProbability(game, mlb) {
  ...
  if (!mlb) {
    const eloAdj = (eloPrior - 0.5) * WEIGHTS.ELO_DIFF_SCALE;
    return clamp(0.5 + eloAdj + parkWinAdj(game.homeTeam), WEIGHTS.PROB_MIN, WEIGHTS.PROB_MAX);
  }
  ...
  const pitcherAdj = (() => {
    const raw = (homePScore - awayPScore) * WEIGHTS.PITCHER_DIFF_SCALE;
  ...
  const lineupAdj  = (homeLineupBlend - awayLineupBlend) * WEIGHTS.LINEUP_DIFF_SCALE;
  ...
  const bullAdj  = (homeBull - awayBull) * WEIGHTS.BULLPEN_DIFF_SCALE;
  ...
  const formAdj  = (homeFormBlend - awayFormBlend) * WEIGHTS.FORM_DIFF_SCALE;

  // 5% — Park factor (run environment)
  const parkAdj = parkWinAdj(game.homeTeam);
  const avgHrFactor = getHrFactor(game.homeTeam);
  const hrAdj = (avgHrFactor - 1.0) * WEIGHTS.HR_FACTOR_SCALE;
