...
export function getModelProbability(game, mlb) {
  ...
  // 22% — Lineup vs pitcher handedness + Savant quality blend
  ...
  // When lineups are posted, blend in Savant wOBA quality (season baseline) at 25%
  ...
  const lineupAdj  = (homeLineupBlend - awayLineupBlend) * 0.11;

  // 20% — Bullpen (equal weight to SP — relievers finish most games; fatigue applied inside bullpenScore)
  const homeBull = bullpenScore(mlb.homeBullpen);
  const awayBull = bullpenScore(mlb.awayBullpen);
  const bullAdj  = (homeBull - awayBull) * 0.10;
  ...
  // 13% — Recent form: blend 10-day (stability) and 7-day (hot streak sensitivity).
  // 7-day gets 45% weight — enough to capture a team on a genuine hot streak without
  // overreacting to a single good series (small sample volatility).
  ...
  const homeFormBlend = homeForm10 * 0.55 + homeForm7 * 0.45;
  const awayFormBlend = awayForm10 * 0.55 + awayForm7 * 0.45;
  const formAdj  = (homeFormBlend - awayFormBlend) * 0.065;

  // 10% — Park factor (run environment)
  const parkAdj = parkWinAdj(game.homeTeam);

  // HR park factor nudge: parks with strong HR skew toward one handedness add tail risk.
  // Averaged across both hands since we don't always know lineup composition.
  const avgHrFactor = getHrFactor(game.homeTeam);
  const hrAdj = (avgHrFactor - 1.0) * 0.012;  // ±1.2% per unit of HR factor vs neutral

  // 5% — Elo
  const eloAdj = (eloPrior - 0.5) * 0.05;

  const total = 0.5 + pitcherAdj + lineupAdj + bullAdj + standAdj + formAdj + parkAdj + hrAdj + eloAdj;
