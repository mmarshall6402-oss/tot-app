...
async function fetchRecentForm(teamId) {
  try {
    const res = await fetch(
      `${MLB_API}/teams/${teamId}/stats?stats=gameLog&group=hitting&season=2025&limit=10`
    );
    const data = await res.json();
...
async function fetchHomeAwaySplit(teamId, isHome) {
  try {
    const res = await fetch(
      `${MLB_API}/teams/${teamId}/stats?stats=statSplits&group=hitting&season=2025&sitCodes=${isHome ? "h" : "a"}`
    );
    const data = await res.json();
    const splits = data?.stats?.[0]?.splits || [];
    if (splits.length === 0) return 0;
...
async function fetchPitcherScore(teamId) {
  try {
    const schedRes = await fetch(
      `${MLB_API}/schedule?sportId=1&hydrate=probablePitcher(note)&season=2025&teamId=${teamId}`
    );
    ...
    const statRes = await fetch(
      `${MLB_API}/people/${pitcher.id}/stats?stats=season&group=pitching&season=2025`
    );
...
async function fetchInjuryAdjustment(teamId) {
  try {
    const res = await fetch(
      `${MLB_API}/teams/${teamId}/roster?rosterType=injuredList&season=2025`
    );
    const data = await res.json();
