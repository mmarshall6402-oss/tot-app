...
import { removeVig, decimalToImplied, americanToDecimal, probToAmericanOdds } from "./edge.js";
...
/**
 * Reprices a Poisson count-stat prop (pitcher_k today; any future market
 * whose lambda is "expected count of the stat this game" can reuse this
 * unchanged) at a hypothetical line the sportsbook never posted. There's no
 * real market price at that line, so the odds returned are model-derived
 * fair odds, not a bettable one — callers should label them as such.
 * batter_hr's lambda is a different shape (P(any HR), not an expected count)
 * and needs its own reprice function if it ever gets a slider.
 * @param {number} lambda - the pick's own projected rate (e.g. expected Ks)
 * @param {number} line - hypothetical line to reprice at
 */
export function repriceAtLine(lambda, line) {
  if (lambda == null || line == null || line < 0) return null;
  const rawOverProb = 1 - poissonCdf(Math.floor(line), lambda);
  const higherPicked = rawOverProb >= 0.5;
  const pickProb = higherPicked ? rawOverProb : 1 - rawOverProb;
  return {
    line,
    pick: higherPicked ? "higher" : "lower",
    modelProb: parseFloat(pickProb.toFixed(4)),
    confidencePct: Math.round(pickProb * 100),
    odds: probToAmericanOdds(pickProb),
  };
}
