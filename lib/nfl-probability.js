...
// Combined-score standard deviation is wider than the margin model's SPREAD_SIGMA —
// two teams' point totals compound variance rather than partially canceling it out
// the way a margin does.
const TOTAL_SIGMA = 10.5;

export function getNFLExpectedTotal(game, nfl) {
  const { homePoints, awayPoints } = getNFLExpectedTeamPoints(game, nfl);
  return homePoints + awayPoints;
}

// Probability the combined final score goes OVER the market total line.
export function getNFLTotalOverProbability(game, nfl, totalLine) {
  if (totalLine == null) return null;
  const expectedTotal = getNFLExpectedTotal(game, nfl);
  return clamp(normCdf((expectedTotal - totalLine) / TOTAL_SIGMA), 0.05, 0.95);
}
