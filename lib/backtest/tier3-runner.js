...
// useCalibration: opt-in checkpoint flag (default false = unchanged production
// behavior, raw getModelProbability). When true, swaps in
// getCalibratedModelProbability everywhere modelProbRaw is used (pick-side
// selection AND the value passed into applyFilterLayer) — this is what
// production would do IF the three live call sites were switched over per
// the calibration rollout plan; NOT what production does today.
export function runTier3({ season = "2025", useCalibration = false, calibrationCurve = null } = {}) {
  ...
  if (useCalibration && calibrationCurve) setCalibrationCurve(calibrationCurve);
  ...
  for (const feat of seasonFeatures) {
    ...
    const modelProbRaw = useCalibration
      ? getCalibratedModelProbability({ homeTeam: official.homeTeam, awayTeam: official.awayTeam }, feat.mlb)
      : getModelProbability({ homeTeam: official.homeTeam, awayTeam: official.awayTeam }, feat.mlb);
