...
import { getModelProbability, setEloRatings, WEIGHTS, computeProbabilityFromComponents } from "../probability.js";
...
// ─── Component-based search (live-data-aware, non-mutating) ────────────────
//
// Same coordinate-ascent algorithm as searchWeights above, but scores
// candidates via computeProbabilityFromComponents against precomputed
// {components, outcome} rows instead of recomputing getModelProbability from
// raw mlb stats objects. Two consequences that matter for
// lib/backtest/run-weight-tuning.js (the automated live version):
//   1. It never mutates the shared WEIGHTS singleton — safe to call from a
//      cron/request context that might run concurrently with live traffic
//      reading WEIGHTS, unlike searchWeights above.
//   2. Historical replay rows and live resolved-pick rows share the exact
//      same {components, outcome, season} shape (see
//      lib/backtest/historical-weight-components.js and live-weight-rows.js),
//      so the same search can blend both — the live signal calibration was
//      missing before app/api/cron/recalibrate existed.
function brierForWeights(rows, weights) {
  const preds = rows.map(r => ({ p: computeProbabilityFromComponents(r.components, weights), outcome: r.outcome }));
  return brierScore(preds);
}

// rows: [{ components, outcome, season }]. baseline: a full WEIGHTS-shaped
// object to start the search from (the currently active vector, not
// necessarily the hardcoded defaults). paramSpecs: same shape as searchWeights.
export function searchWeightsFromComponents({ rows, baseline, paramSpecs, passes = 2 }) {
  let current = { ...baseline };
  let currentBrier = brierForWeights(rows, current);
  const baselineBrier = currentBrier;
  const log = [];
  ...
  for (let pass = 0; pass < passes; pass++) {
    for (const { name, candidates } of paramSpecs) {
      ...
      for (const candidate of candidates) {
        if (candidate === bestValue) continue;
        const trialBrier = brierForWeights(rows, { ...current, [name]: candidate });
        if (trialBrier < bestBrier - 1e-9) {
          bestValue = candidate;
          bestBrier = trialBrier;
          improved = true;
        }
      }

      if (improved) {
        log.push({ pass, param: name, from: current[name], to: bestValue, before: currentBrier, after: bestBrier });
        current[name] = bestValue;
        currentBrier = bestBrier;
      }
    }
  }

  return { weights: current, brier: currentBrier, baselineBrier, changes: log };
}
