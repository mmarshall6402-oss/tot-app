...
export function runTier2({ seasons } = {}) {
  ...
  const evalRows = rows.filter(r => r.season === "2024");

  const controlPoints = isotonicFit(trainRows.map(r => ({ x: r.modelProb, y: r.outcome })));
  const evalPredsRaw = evalRows.map(r => ({ p: r.modelProb, outcome: r.outcome }));
  const evalPredsCalibrated = evalRows.map(r => ({ p: isotonicPredict(controlPoints, r.modelProb), outcome: r.outcome }));
  ...
  const metrics = {
    ...
    holdout2024: {
      beforeCalibration: summarize(evalPredsRaw),
      afterIsotonicCalibration: summarize(evalPredsCalibrated),
    },
  };
  ...
  return {
    ...
    gameCount: rows.length,
    params: {
      calibrationSplit: "train=2022,2023; holdout=2024",
      seasons: seasons ?? "all",
    },
    metrics,
    rows: rows.map(r => ({
      ...
      model_home_prob_calibrated: r.season === "2024" ? isotonicPredict(controlPoints, r.modelProb) : null,
      features: r.features,
    })),
  };
}
