...
export function runTier2({ seasons } = {}) {
  ...
  const summarizeHoldout = evalRows => ({
    ...
    afterIsotonicCalibration: summarize(evalRows.map(r => ({ p: isotonicPredict(controlPoints, r.modelProb), outcome: r.outcome }))),
  });
  ...
  return {
    ...
    rows: rows.map(r => ({
      ...
      model_home_prob_calibrated: (r.season === "2024" || r.season === "2025") ? isotonicPredict(controlPoints, r.modelProb) : null,
      features: r.features,
    })),
  };
}
