// app/admin/fantasy-personnel/page.js
// Upload a team personnel-usage screenshot (e.g. projected % of snaps in 3+
// WR sets vs 2-or-fewer WR sets, one row per team) — Claude's vision
// capability extracts it and it's folded season-long into WR/TE Cheat Sheet
// rankings via lib/nfl-fantasy/personnel-adjustment.js.
...
export default function FantasyPersonnelImport() {
  ...
  const upload = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/admin/fantasy-personnel-import", {
        method: "POST",
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#000", color: "#fff", padding: "24px 20px", fontFamily: "system-ui, sans-serif" }}>
      <div style={{ fontSize: 18, fontWeight: 700, marginBottom: 4 }}>T<span style={{ color: "#00FF87" }}>|</span>T Fantasy Personnel Import</div>
      ...
      <div style={{ marginTop: 24, maxWidth: 480 }}>
        <div style={{ fontSize: 12, color: "#888", marginBottom: 16, lineHeight: 1.6 }}>
          Upload a team personnel-usage screenshot (one row per team: % of snaps in 3+ WR sets vs
          2-or-fewer WR sets). Claude extracts the data and folds it season-long into WR and TE
          Cheat Sheet rankings &mdash; more 3+ WR usage favors WR2/WR3/slot value, more 2-or-fewer WR
          usage favors TE value.
        </div>
        ...
        {result && (
          <div style={{ color: "#00FF87", fontSize: 12, marginTop: 12 }}>
            Extracted {result.teamsExtracted} teams, stored {result.rowsStored} rows.
          </div>
        )}
      </div>
    </div>
